	...
	if ( ! $db->exists( $sql ) ) {
		...
		if ( preg_match( '/^[a-zA-Z0-9\-_. ]+$/', $_POST['alias'] ) ) {
			...
			if ( $_POST['password1'] == $_POST['password2'] ) {
				...
				debug("error: passwords don't match");
			}
		} else {
			debug("error: invalid alias");
		}
	} else {
		debug("error: username exists");
	}
} else {
	debug("error: invalid username");
}
